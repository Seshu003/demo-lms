...
// ── 4-Agent pipeline ──────────────────────────────────
// Each agent has its own index so the server can route it to a separate API key.
// Mode/length are baked strongly into the prompts — not just mentioned.
function buildAgents(topic, mode, length) {
  ...
  return [
    {
      name: "Topic Planner", icon: Target, desc: "Mapping the concept landscape", color: T.accent,
    ...
    {
      name: "Requirement Developer", icon: Layers, desc: "Building explanation requirements", color: T.green,
    ...
    {
      name: "Output Optimizer", icon: Zap, desc: "Optimising for your learning style", color: T.amber,
    ...
    {
      name: "Theory Explainer", icon: Lightbulb, desc: "Generating your personalised answer", color: T.purple,
...
function parseOutput(text) {
  // Extract a section between two labels (or to end of string)
  ...
  // Infographic points — lines starting with - or •
  const infoPoints = infoRaw
    .split('\n')
    .map(l => l.replace(/^[\-•*]\s*/, '').trim())
    .filter(l => l.length > 2);

  // Quiz questions — split on Q1:, Q2: etc.
  const quizQuestions = [];
  ...
  for (const block of qBlocks) {
    ...
    // Options can be on one line: "A) foo B) bar C) baz D) qux"
